...
import {
  listSlackMessageJobsFiltered,
  parseSlackMessageJobSort,
  type SlackMessageJobListFilters,
  type SlackMessageJobSort,
} from "@/lib/operations-queries";

type Ok = {
  success: true;
  data: Awaited<ReturnType<typeof listSlackMessageJobsFiltered>>["rows"];
  total: number;
  limit: number;
  offset: number;
};
...
function parseSort(v: string | string[] | undefined): SlackMessageJobSort {
  const s = (Array.isArray(v) ? v[0] : v)?.trim() ?? "";
  const parsed = parseSlackMessageJobSort(s);
...
export default async function handler(
  ...
  try {
    ...
    const ckRaw = q.channel_kind;
    const ckStr = Array.isArray(ckRaw) ? ckRaw[0] : ckRaw;
    const channelKind =
      ckStr === "user" || ckStr === "channel" ? ckStr : undefined;

    const filters: SlackMessageJobListFilters = {
      ...
      channelKind,
      channelContains:
        typeof q.channel === "string" && q.channel.trim() ? q.channel : undefined,
      bullJobIdContains:
    ...
    const { rows, total } = await listSlackMessageJobsFiltered(filters);
    ...
    console.error("slack_message_jobs fetch error:", error);
    return res.status(500).json({
      success: false,
      message: "Failed to fetch Slack message jobs",
    });
  }
}
